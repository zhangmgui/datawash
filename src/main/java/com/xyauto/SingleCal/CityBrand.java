...
public class CityBrand {
    public void computeCityBrand(QueryRunner qr) throws SQLException {
        long t1 = System.currentTimeMillis();
        List<MediaParamDomain> r1 = qr.query(QuerySqlUtils.cityBrandQuery, new BeanListHandler<MediaParamDomain>(MediaParamDomain.class));
        long t2 = System.currentTimeMillis();
        System.out.println("城市品牌查询耗时：" + ((t2 - t1) / (1000)) + "秒");
        calculateOneNum(true,r1);
        calculateFiveIndex(r1);
        /*calculateLevel(r1);*/
        calculateLevel(true,r1);
        long t3 = System.currentTimeMillis();
        System.out.println("城市品牌计算耗时：" + ((t3 - t2) / (1000)) + "秒");
        insertParamMethod(r1, qr);
        long t4 = System.currentTimeMillis();
        System.out.println("城市品牌中间数据入库耗时：" + ((t4 - t3) / (1000)) + "秒");

        insertIndexMethod(r1, qr);
        long t5 = System.currentTimeMillis();
        System.out.println("城市品牌间数据入库耗时：" + ((t5 - t4) / (1000)) + "秒");
    }
    ...
    private void insertParamMethod(List<MediaParamDomain> r1, QueryRunner qr) throws SQLException {
        for (MediaParamDomain someData : r1) {
            qr.update(InsertSqlUtils.cityBrandInsertMediaSQL,
                    someData.getDimDate(),
                    someData.getDimCar(),
                    someData.getDimArea(),
    ...
    private void insertIndexMethod(List<MediaParamDomain> r1, QueryRunner qr) throws SQLException {
        for (MediaParamDomain someData : r1) {
            qr.update(InsertSqlUtils.cityBrandInsertIndexSQL,
                    someData.getDimDate(),
                    someData.getDimCar(),
                    someData.getDimArea(),
                    someData.getProvince(),
