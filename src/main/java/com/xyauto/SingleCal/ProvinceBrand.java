 ...
 */
public class ProvinceBrand {
    public void computeProBrand(QueryRunner qr) throws SQLException {
        long t1 = System.currentTimeMillis();
        List<MediaParamDomain> r1 = qr.query(QuerySqlUtils.proBrandQuery, new BeanListHandler<MediaParamDomain>(MediaParamDomain.class));
        long t2 = System.currentTimeMillis();
        System.out.println("省份品牌查询耗时：" + ((t2 - t1) / (1000)) + "秒");
        calculateOneNum(false,r1);
        calculateFiveIndex(r1);
        calculateLevel(false,r1);
        long t3 = System.currentTimeMillis();
        System.out.println("省份品牌计算耗时：" + ((t3 - t2) / (1000)) + "秒");
        insertParamMethod(r1, qr);
        long t4 = System.currentTimeMillis();
        System.out.println("省份品牌中间数据入库耗时：" + ((t4 - t3) / (1000)) + "秒");

        insertIndexMethod(r1, qr);
        long t5 = System.currentTimeMillis();
        System.out.println("省份品牌间数据入库耗时：" + ((t5 - t4) / (1000)) + "秒");
    }
    ...
    private void insertParamMethod(List<MediaParamDomain> r1, QueryRunner qr) throws SQLException {
        for (MediaParamDomain someData : r1) {
            qr.update(InsertSqlUtils.proBrandInsertMediaSQL,
                    someData.getDimDate(),
                    someData.getDimCar(),
                    someData.getDimArea(),
                    someData.getSaleVol(),
    ...
    private void insertIndexMethod(List<MediaParamDomain> r1, QueryRunner qr) throws SQLException {
        for (MediaParamDomain someData : r1) {
            qr.update(InsertSqlUtils.proBrandInsertIndexSQL,
                    someData.getDimDate(),
                    someData.getDimCar(),
                    someData.getDimArea(),
